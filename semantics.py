...
def sem(tr):
    ...
    # When the tree label is P, N, A, I or T simply apply unary or binary logical lambda expression
    if tr.label() == 'P':
        return tr[0][0]
    elif tr.label() == 'N':
        return '(\\x.' + tr[0][0] + '(x))'
    ...
    # Reading the top level rules
        ...
        # Looks into the NP (subject of transitive verb) and checks
        # whether if it's a proper noun or not.
        # Forward lookup is necessary since the lambda expressions
        # for P with other rules of NP are not in the same form.
